...
3. Fügen Sie das `grafana` Helm-Repository hinzu:
   ```shell
   helm repo add grafana https://grafana.github.io/helm-charts
   helm repo update
   ```
4. Installieren Sie das Grafana Helm-Chart mit unseren [benutzerdefinierten Werten](grafana-values.yaml):
   ```shell
...
5. Extrahieren Sie das Grafana-Administrator-Passwort:
   ```shell
   kubectl get secret -n monitoring external-grafana \
     -o jsonpath="{.data.admin-password}" \
     | base64 --decode ; echo
   ```
6. Machen Sie die Grafana-Instanz lokal verfügbar:
   ```shell
