...
3. Restart `k8s-prometheus`:
   ```shell
   kubectl rollout restart statefulset --selector=k8s.cloudogu.com/component.name=k8s-prometheus -n ecosystem
   ```
4. Add the `grafana` Helm repository:
   ```shell
   helm repo add grafana https://grafana.github.io/helm-charts
   helm repo update
   ```
5. Deploy the Grafana Helm chart with our [custom values](grafana-values.yaml):
   ```shell
...
6. Extract the Grafana admin password:
   ```shell
   kubectl get secret -n monitoring external-grafana \
     -o jsonpath="{.data.admin-password}" \
     | base64 --decode ; echo
   ```
7. Expose the Grafana instance locally:
   ```shell
